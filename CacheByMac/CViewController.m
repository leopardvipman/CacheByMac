...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    button=[[UIButton alloc]initWithFrame:CGRectMake(cell.imageView.frame.size.width+80, cell.imageView.frame.size.height, 100, 40)];
   
    button.backgroundColor=[UIColor redColor];
 
    NSString *urlstring=[_objects objectAtIndex:indexPath.row+1];
    [cell addSubview:[self imageFromURLString:urlstring]];//在这里加载图片
    UIImageView *imageview=[self imageFromURLString:urlstring];
    UIImage *imagenow=imageview.image;
       button.backgroundColor=[UIColor colorWithPatternImage:imagenow];
...
- (UIImageView *) imageFromURLString: (NSString *) Value
{
    //    return [UIImage imageWithData:[NSData dataWithContentsOfURL:[NSURL URLWithString:Value]]];
    UIImageView *imgthis=[[UIImageView alloc]init];
    [imgthis setImageWithURL:[NSURL URLWithString:Value] placeholderImage:[UIImage imageNamed:@"placeholder"]];
    return imgthis;
}
